...
CSS_LINKS = """    <!-- FruitBridge CV3 Design System CSS -->
...
def extract_inline_styles(html_content):
    ...
    for tag in style_tags:
        if tag.string:
            styles.append(tag.string.strip())
...
def update_html_file(file_path, category):
    ...
    old_links = soup.find_all('link', rel='stylesheet')
    for link in old_links:
        href = link.get('href', '')
        if 'design-system.css' in href or 'components.css' in href:
            link.decompose()
    ...
    if head:
        ...
        if title:
            # title 태그 다음에 CSS 링크 추가
            new_css = CSS_LINKS
            if category in CATEGORY_CSS_MAP:
                new_css += f'\n    <link rel="stylesheet" href="../../styles/{CATEGORY_CSS_MAP[category]}">'

            # BeautifulSoup으로 새 요소 생성
            new_elements = BeautifulSoup(new_css, 'html.parser')

            # title 다음에 삽입
            for elem in reversed(new_elements.contents):
                if elem.name:
                    title.insert_after(elem)
        ...
        if category_dir.is_dir():
            ...
            for html_file in category_dir.glob('**/*.html'):
                try:
                    ...
                    processed += 1
                    print(f"✅ Updated: {html_file.relative_to(PAGES_DIR)}")

                except Exception as e:
                    errors.append(f"❌ Error processing {html_file}: {e}")
...
def main():
    """메인 실행 함수"""
    print("=" * 60)
    print("FruitBridge CSS 분리 작업 시작")
    print("=" * 60)
    ...
    print("작업 완료 요약")
    print("=" * 60)
    print(f"✅ 생성된 CSS 파일: {len(css_files)}개")
    print(f"✅ 업데이트된 HTML 파일: {processed}개")
